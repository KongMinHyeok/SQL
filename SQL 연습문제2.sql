...
insert into `bank_customer` value ("730423-1000001","김유신",2,"010-1234-1001","경기도 수원시");
insert into `bank_customer` value ("750210-1000002","김춘추",1,"010-1234-1002","경기도 광주시");
insert into `bank_customer` value ("830513-2000003","선덕여왕",2,"010-1234-1003","서울시 마포구");
insert into `bank_customer` value ("870830=1000004","강감찬",2,"010-1234-1005","강원도 강릉시");
insert into `bank_customer` value ("941127-1000006","이순신",2,"010-1234-1006","부산시 영도구");
...
insert into `bank_account` value ("101-11-2001","S1","자유저축예금","220-82-52237",230000000,"2003-04-14");
...
insert into `bank_account` value ("101-11-1002","S2","정기적립예금","830513-2000003",1020000,"2005-07-21");
insert into `bank_account` value ("101-11-1005","S3","주택청약예금","941127-1000006",720800,"2005-07-21");
insert into `bank_account` value ("101-11-1004","L1","고객신용대출","910912-2000005",1200500,"2005-07-21");
insert into `bank_account` value ("101-11-1006","L2","예금담보대출","730423-1000001",25000,"2005-07-21");
insert into `bank_account` value ("101-11-1007","L3","주택담보대출","750210-1000002",2700000,"2005-07-21");
...
insert into `bank_transaction` (`t_a_no`, `t_dist`, `t_amount`, `t_datetime`)values("101-11-1005",1,200000,now());
...
#실습2-17
select * from `bank_account` where `a_item_name`='자유저축예금'  order by `a_balance` asc;

#실습2-18
select * from `bank_account` where `a_item_name`='자유저축예금' order by `a_balance` limit 1;

#실습2-19
select * from `bank_transaction` where `t_dist` not like '3'  order by`t_dist` , `t_amount` desc;

#실습2-20
select count(if(`t_dist` =1, 1, null)) as `입금 건수`,
	   count(case when `t_dist`='2' then 2 end) as `출금 건수`, 
       count(case when `t_dist`='3' then 3 end) as `조회 건수` from `bank_transaction`;
...
join `bank_customer` as c on a.t_dist = c.c_dist
where `t_dist` =1
...
select 	
	...
	`a_item_name`,
	`c_name`,
	COUNT(`t_no`) AS `거래건수` from `bank_transaction` as a
join `bank_account` as b on a.t_a_no = b.a_no
join `bank_customer` as c on a.t_dist = c.c_dist
